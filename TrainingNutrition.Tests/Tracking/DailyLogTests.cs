...
public sealed class DailyLogTests
{
    ...
    {
        var log = new DailyLog(new DateOnly(2026, 2, 21));
    ...
    public void GetTotalMacros_ShouldSumMealsMacros()
    {
        ...
        meal1.AddDish(CreateSimpleDish("Dish 1", protein: 10m, carbs: 20m, fat: 5m, grams: 100));

        // Meal 2: one dish => 5/0/10
        var meal2 = new Meal(MealType.Meal, "Meal 2", DateTimeOffset.UtcNow);
        meal2.AddDish(CreateSimpleDish("Dish 2", protein: 5m, carbs: 0m, fat: 10m, grams: 100));

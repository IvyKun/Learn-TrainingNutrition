...
public sealed class MacronutrientsTests
{
    ...
    public void Constructor_ShouldThrow_WhenAnyMacroIsNegative(decimal protein, decimal carbs, decimal fat, decimal fiber, decimal salt)
    {
        var act = () => new Macronutrients(protein, carbs, fat, fiber, salt);
    ...
    [Theory]
    [InlineData(0, 0, 2.5, 0, 0, 23)]     // 22.5 -> 23 (AwayFromZero)
    [InlineData(0, 0, 2.4, 0, 0, 22)]     // 21.6 -> 22
    [InlineData(0, 0, 2.6, 0, 0, 23)]     // 23.4 -> 23
    [InlineData(1, 0, 0, 0, 0, 4)]
    [InlineData(0, 1, 0, 0, 0, 4)]
    [InlineData(0, 0, 1, 0, 0, 9)]
    public void Calories_ShouldRound_AwayFromZero(decimal protein, decimal carbs, decimal fat, decimal fiber, decimal salt, int expectedCalories)
    {
        var macros = new Macronutrients(protein, carbs, fat, fiber, salt);
    ...
    {
        var a = new Macronutrients(10m, 20m, 5m, 0m, 0m);
        var b = new Macronutrients(5m, 0m, 10m, 0m, 0m);

        var result = a.Add(b);

        Assert.Equal(new Macronutrients(15m, 20m, 15m, 0m, 0m), result);
    ...
    {
        var a = new Macronutrients(10m, 20m, 5m, 0m, 0m);
        var b = new Macronutrients(10m, 20m, 5m, 0m, 0m);
    ...
    {
        var a = new Macronutrients(10m, 20m, 5m, 0m, 0m);
        var b = new Macronutrients(10m, 20.1m, 5m, 0m, 0m);
